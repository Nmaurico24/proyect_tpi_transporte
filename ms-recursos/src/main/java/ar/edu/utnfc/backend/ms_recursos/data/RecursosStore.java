...
public class RecursosStore {
    ...
    // CAMIONES
    private final Map<UUID, Camion> camionesData = new ConcurrentHashMap<>();
    private final Map<UUID, Deposito> depositosData = new ConcurrentHashMap<>();
    private final Map<UUID, Tarifa> tarifasData = new ConcurrentHashMap<>();
    private final Map<UUID, TarifaCamion> tarifaCamionData = new ConcurrentHashMap<>();
    private final Map<UUID, TarifaDeposito> tarifaDepositoData = new ConcurrentHashMap<>();
    private final Map<UUID, CostoEstimacion> costosEstimacionData = new ConcurrentHashMap<>();
    private final Map<UUID, CostoFinal> costosFinalData = new ConcurrentHashMap<>();
    ...
    private static final List<String[]> DEPOSITOS_DATA = List.of(
            ...
            new String[] { "Depósito Oeste", "Av. Armada Argentina 2800, Córdoba", "-31.432167", "-64.278901",
                    "710.00" });

    // ----- Datos de prueba para Tarifas -----
    private static final List<String[]> TARIFAS_DATA = List.of(
            // description, unidad, valor, moneda, vigencia_vigenciaHasta , version
    ...
    private void seedTarifas() {
        ...
        for (String[] tarifaData : TARIFAS_DATA) {
            ...
            tarifa.setVigenciaHasta(java.time.LocalDate.parse(tarifaData[4]));
    ...
    private void seedTarifasCamiones() {
        ...
        for (int i = 0; i < Math.min(camionIds.size(), tarifaIds.size()); i++) {
            TarifaCamion tarifaCamion = new TarifaCamion();
    ...
    private void seedTarifasDepositos() {
        ...
        for (int i = 0; i < Math.min(depositoIds.size(), tarifaIds.size()); i++) {
            TarifaDeposito tarifaDeposito = new TarifaDeposito();
    ...
    public List<Camion> findAllCamiones(Boolean active) {
        return camionesData.values().stream()
                .filter(c -> active == null || c.getLsActive().equals(active))
                .sorted(Comparator.comparing(Camion::getDominio))
                .toList();
    }

    public Optional<Camion> findCamionById(UUID id) {
        return Optional.ofNullable(camionesData.get(id));
    }

    public List<Deposito> findAllDepositos(Boolean active) {
        return depositosData.values().stream()
                .filter(d -> active == null || d.getLsActive().equals(active))
                .sorted(Comparator.comparing(Deposito::getNombre))
                .toList();
    }

    public Optional<Deposito> findDepositoById(UUID id) {
    ...
    public Optional<Tarifa> findTarifaById(UUID id) {
        return Optional.ofNullable(tarifasData.get(id));
    }

    public List<TarifaCamion> findTarifasByCamion(UUID camionId) {
        return tarifaCamionData.values().stream()
                .filter(tc -> tc.getCamionId().equals(camionId))
                .toList();
    }

    public List<TarifaDeposito> findTarifasByDeposito(UUID depositoId) {
    ...
    public void saveTarifaCamion(TarifaCamion tarifaCamion) {
        tarifaCamionData.put(tarifaCamion.getId(), tarifaCamion);
    }

    public void saveTarifaDeposito(TarifaDeposito tarifaDeposito) {
        tarifaDepositoData.put(tarifaDeposito.getId(), tarifaDeposito);
    }

    public List<CostoEstimacion> findAllCostosEstimacion() {
    return new ArrayList<>(costosEstimacionData.values());
}
public List<CostoFinal> findAllCostosFinal() {
    return new ArrayList<>(costosFinalData.values());
}

public void saveCostoEstimacion(CostoEstimacion costoEstimacion) {
    costosEstimacionData.put(costoEstimacion.getId(), costoEstimacion);
}

public void saveCostoFinal(CostoFinal costoFinal) {
    costosFinalData.put(costoFinal.getId(), costoFinal);
}
